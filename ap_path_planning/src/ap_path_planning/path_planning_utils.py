...
class FormCalculator(object):
    ...
    def __init__(self, owner_behavior, own_id, swarm):
        ...
        if not isinstance(owner_behavior, behavior.Behavior):
            raise Exception("FormCalculator owner must be Behavior")
    ...
    def set_params(self, ldr_id, distance, angle, alt, alt_mode):
        ...
        if not ldr_id in self._swarm:
            ...
            self._is_set = False
        elif alt_mode != FormCalculator.BASE_ALT_MODE and \
             alt_mode != FormCalculator.ALT_SEP_MODE:
    ...
    def compute_intercept_waypoint(self):
        ...
        if not self._is_set:
            ...
            return None
        if self._own_id not in self._swarm:
            self._is_set = False
            self._owner.log_warn("Cannot compute waypoint--own ID not in swarm")
            return None

        own_uav = self._swarm[self._own_id]
        lead_uav = self._swarm[self._lead_uav_id]
        if (rospy.Time.now() - lead_uav.state.header.stamp) > \
           FormCalculator.MAX_TIME_LATE:
            self._owner.log_warn("Cannot compute waypoint--lead UAV time late")
            return None

        # Grab & compute required own & leader state info
        own_lat = own_uav.state.pose.pose.position.lat
        own_lon = own_uav.state.pose.pose.position.lon
        own_crs = math.atan2(own_uav.state.twist.twist.linear.y, \
                             own_uav.state.twist.twist.linear.x)
        own_spd = math.hypot(own_uav.state.twist.twist.linear.x, \
                             own_uav.state.twist.twist.linear.y)
        ...
        time_to_intercept = \
            min(time_to_intercept, FormCalculator.MAX_LOOKAHEAD)
        ...
        wpt.lat, wpt.lon = \
            gps.gps_newpos(own_lat, own_lon, to_tgt, FormCalculator.OVERSHOOT)
        if self._alt_mode == FormCalculator.BASE_ALT_MODE:
            wpt.alt = self._alt
        else:
